	...
	connection.on('message', function(message) {
		...
		switch (eventName) {
			...
			case "loginSuccess":
				...
				playerInfoMap[recieveData.playerId] = recieveData;
				break;
			// ----------------------------------------------------------------------
			// クライアントの操作情報を受け付ける処理.
			// ----------------------------------------------------------------------
			case "setPlayerAction":
				var tmp = playerInfoMap[recieveData.playerId];
				if (tmp) {
			...
			case "sendHostInfo":
				...
				sendAll(eventName, obj);
				//send(connection, eventName, obj);
				//sendWithoutSelf(connection, "setPlayerAction", obj);
				send(browser, "host", obj);
				break;
		}
	});
	// ----------------------------------------------------------------------
